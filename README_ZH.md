...
- **统一 MCP, SKILLS 管理** — 一个面板管理 Claude、Codex、Gemini、OpenCode 和 Hermes 的 MCP、Skills，支持双向同步，并额外支持内部 `codefree-o` CLI 的 MCP 同步
...
- **统一 MCP 面板** — 管理 Claude、Codex、Gemini、OpenCode 和 Hermes 的 MCP 服务器，双向同步，支持 Deep Link 导入，并可额外同步到内部 `codefree-o` CLI
...
- 浏览、搜索和恢复全部支持来源的对话历史，包含内部 `codefree-o` CLI 历史记录
...
如需了解各项功能的详细使用方法，请查阅 **[用户手册](docs/user-manual/zh/README.md)** — 涵盖供应商管理、MCP/Prompts/Skills、会话历史、代理与故障转移等全部功能。
...
- **MCP**：点击"MCP"按钮 → 通过模板或自定义配置添加服务器 → 切换 Claude / Codex / Gemini / OpenCode / 内部 `codefree-o` 的同步开关
- **Prompts**：点击"Prompts" → 使用 Markdown 编辑器创建预设 → 激活后同步到 live 文件
- **Skills**：点击"Skills" → 浏览 GitHub 仓库 → 一键安装到支持的应用
- **会话**：点击"Sessions" → 浏览、搜索和恢复全部支持来源的对话历史，包含内部 `codefree-o` CLI
