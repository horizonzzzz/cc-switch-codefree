...
| codefree-o | 同步到内部 codefree-o CLI | `~/.codefree-o/.config/codefree.json` 的 `mcp` |
| OpenCode | 同步到 OpenCode | `~/.config/opencode/opencode.json` 的 `mcp` |
| Hermes | 同步到 Hermes | `~/.hermes/config.yaml` 的 `mcp_servers` |

> ⚠️ **注意**：OpenClaw 和 Claude Desktop 暂不支持 CC Switch MCP 同步。MCP 功能支持 Claude、Codex、Gemini、OpenCode 和 Hermes，并额外为内部 `codefree-o` CLI 提供 MCP 同步支持；`codefree-o` 只支持 MCP，不参与供应商配置、Skills 或连通性检测。

### 开关实现机制

当开启某个应用的开关时，CC Switch 会：

1. **更新数据库**：将服务器的 `apps.claude/codex/gemini/opencode/hermes/codefree-o` 状态设为 `true`
...
- **codefree-o**：需存在 `~/.codefree-o/.config/` 目录，或在设置中覆盖 `codefree-o` 配置目录
...
> 当前分支中的内部 `codefree-o` CLI 已支持 MCP 同步写入，但暂未接入「从现有配置导入」入口。
...
### codefree-o (`~/.codefree-o/.config/codefree.json`)
...
> `codefree-o` 沿用独立的 `mcp` 字段而不是顶层 `mcpServers`。如果你在设置里覆盖了 `codefree-o` 配置目录，CC Switch 会把配置写到该目录下的 `codefree.json`。
